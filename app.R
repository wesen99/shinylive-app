...
        }
        .hoverCardTitle{ font-weight:900; font-size:13px; line-height:1.25; margin:0 0 4px 0; color:#0f172a; }
        .hoverCardType{ font-size:12px; color:#64748b; margin:0 0 8px 0; }
        .hoverRow{ display:flex; gap:10px; align-items:flex-start; }
        .hoverImg{ width:92px; height:62px; border-radius:12px; object-fit:cover; border:1px solid rgba(0,0,0,.08); background:#f1f5f9; flex:0 0 auto; }
        .hoverMeta{ font-size:12px; line-height:1.35; color:#0f172a; }
        .hoverMeta span{ color:#64748b; font-weight:700; }
          
        /* charts */
        .plotWrap{ background:#fff; border-radius:14px; border:1px solid rgba(0,0,0,.06); padding:10px; }
       .hoverRow{
  display:flex;
...
/* ONE CLEAN CONTENT BOX UNDER IMAGE */
.hoverContent{
  background: #ffffff;
...
      "))
    ...
    bs4TabItems(
      ...
      bs4TabItem(
        ...
        fluidRow(
          bs4Dash::box(
            title = "Functionality (Functional vs Not Functional)",
            width = 6, status = "warning", solidHeader = TRUE,
            div(class="plotWrap", plotOutput("plt_func_dist", height = "300px"))
          ),
          bs4Dash::box(
            title = "Budget Utilized by Infrastructure Type (ETB)",
            width = 6, status = "warning", solidHeader = TRUE,
            div(class="plotWrap", plotOutput("plt_budget_by_type", height = "300px"))
          )
        ),
        
        fluidRow(
          bs4Dash::box(
            title = "Not Handed Over — Exceptions",
            width = 6, status = "danger", solidHeader = TRUE,
            DTOutput("tbl_not_handed")
          ),
          bs4Dash::box(
            title = "Top 10 Projects by Beneficiaries (Numeric)",
